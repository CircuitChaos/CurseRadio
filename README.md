...
CurseRadio is a text-mode, ncurses-based program to control a CAT-capable transceiver. It was written with contests in mind, but can be also used as a general-purpose Cabrillo logger (it doesn't support ADI).
...
* -f &lt;file&gt; is used to specify Cabrillo file to append to. If it's not specified, then logging and callsign check functions will be disabled. Logging event appends a QSO line to this file (it doesn't rewrite anything). Callsign check functions reads the file every time. Therefore, it is safe to edit the file in external editor between using these functions (you don't need to exit the program, reload the file, etc.).

* -c &lt;port&gt; and -b &lt;rate&gt; is used to specify CAT serial port and baudrate. FT-891 exposes two serial ports – one is used for CAT control and another for PTT control. If you want to use permanent port names (I use /dev/ttyFTCAT and /dev/ttyFTPTT), see the section called *udev*. If CAT port is not specified, then most program functions won't be enabled.
...
* -p &lt;port&gt; is used to specify the port used to key the radio (for CW) by controlling the DTR line. Note that PC control has to be enabled in radio settings. Also note that when the port is opened, both DTR and RTS lines are momentarily brought up, so the radio will be keyed for a moment when you start the program with this option. This is how the serial driver on Linux works and it can't be changed without modifying the driver code.
...
* -w &lt;wpm&gt; is used to specify the initial speed of the keyer built into the program, in words per minute (WPM). Note that this is **not** the speed of the keyer built into the radio. Due to CAT interface limitations, text to be sent as CW is paced by the program, not by the radio. Speed can be changed during operation and if it's not specified, a default of **25** WPM is used.
...
When the program is started, it displays a black screen with a blue status bar. On the statusbar, frequency, mode and meters are shown (in RX mode it's the signal level, in TX mode it's the IDD (drain current of the final transistors), ALC, compressor level, output power, and of course SWR. Meters are updated every 100 ms (this value is fixed, but might be made editable from the command-line in future versions; see `METER_POLL_INTERVAL` in `curseradio.cpp`). There's also a rotating indicator on the left of the statusbar to show that CAT is working, although after two seconds of no response from the radio a CAT timeout trips and the program exits.

Program is controlled from the keyboard. Press 'h' to see a list of keys, or 'q' to quit. Some keys and their explanations:

* n: allows you to enter a note. It's not saved anywhere, just kept on the screen, but is useful when, for example, you're receiving an exchange and want to write it down before logging it. Just press 'n', type what you like, and press Enter to end this mode.
...
* =: reset frequency to the lowest possible for given band

Hint: frequency limits are defined in `band.cpp` and they're valid for IARU 1 region. If your region has different limits, feel free to change them.

* b: allows to select band. A screen is presented with all bands supported by FT-891, plus a generic band and MW band. Backspace aborts selection.

* m: allows to set mode. CAT manual is very unclear on how modes are mapped to numbers (different numbers map to the same mode, and so on), so if this doesn't work as expected, make sure to fill a bug report. Currently it's impossible to switch between USB and LSB – the mode is always SSB, and sideband is defined in the radio per band. Backspace abort selection.

Note that you don't have to use 'b' or 'm'. Switching the band and mode in the radio is sufficient – the program will read it automatically after at most 100 ms.

* v: swaps VFOs (A to B, and B to A), and their respective modes. Not really useful, implemented because it was very simple to do.
...
* x: shows current (next) exchange. Useful when you want to pick up the microphone and tell the number. This exchange will be then logged (and possibly incremented) when you press 'l'.

* c: checks if the callsign is already in the log. Press 'c', type callsign (lowercase or uppercase, doesn't matter) and the program will scan the CBR file, telling you if the callsign is in the log or not.
...
* l: logs the QSO in Cabrillo format. Your side is logged as your callsign, 59 or 599 (depending on the mode – 599 for CW, 59 for all others), and the next exchange group (the same as shown when pressing 'x'). After 'l' is pressed, you can either specify the remote callsign and received exchange (for example: SP1ZZZ 123), or remote callsign, received report, and received exchange (if the report is something else than 59, or 599 for CW). If you want to leave logging mode, just erase everything and press Enter – nothing will be logged.

* t: sends text as CW. Pressing 'a' will abort sending.

* u and d: increase and decrease built-in CW keyer speed.

* z: zero-in on the CW frequency. It just sends the command to the radio, equivalent to pressing ZIN button.
...
Just type `scons` and press Enter. If everything's OK, a binary will be put in `build/curseradio`. To install it system-wide, type `sudo scons install` – it will be installed in `/usr/local/bin/` directory.
...
This is the list of CAT commands as used by the program. It can be useful when determining if and what things have to be done to adjust it to work with other radios.
...
* RMnxxx: meter number n with value xxx (0 to 255). These values are then translated to meaningful form in `meters.cpp`, and this translation is probably only correct for FT-891
...
If IDD is 0, then it means we're in RX mode. Signal (S-meter) is read, followed by frequency and band, and this information is presented on the status bar.

If IDD is not 0, then it means radio is transmitting. Other meters (ALC, compressor, power, SWR) are read and presented on the status bar. Frequency and mode are **not** read, as it's assumed that they can't (or at least shouldn't) change when TXing. It means that if the program is started when the radio is transmitting, frequency and mode information won't be available.
...
* Decoding of meters other than S-meter doesn't really work – IDD, SWR and ALC is not shown correctly
...
If you want to use this program, do it. Just don't expect anything from me and don't hold me accountable for anything. Think before you do anything – remember that you can kill your radio if you transmit into an unmatched load, etc.

If you want to use parts of this program in your own uncommercial project (open- or closed-source), you're also free to do it. You don't have to list me anywhere (although it would be nice if you provided a link to this program, if you think your users would benefit from it). Sharing is caring.
